...
def getOutlierScores(df):
    ...
    iforest = IsolationForest(
        n_estimators=100, contamination=Settings.outlier_contamination
    )
...
def __drawOutliers(df, columns, outputFile):
    ...
    if len(columns) < 2:
        log.info(
            "Can not generate outlier plot due to insufficient number of attributes (<2)"
        )
        return
    elif len(columns) == 2:
        plotdf = dataDf
        ...
        plotdf = pd.DataFrame(
            PCA(2).fit(dataDf).transform(dataDf), columns=["component_1", "component_2"]
        )
...
def clean(source_file, save_file, columns):
    ...
    if dataDf[columns].shape[1] > Settings.col_limit_for_outlier_plot:
        log.info(
            "Too many variables to plot!! Please sleect the vaiables to plot using combinations argument."
        )
        return
    elif (dataDf.shape[0] * dataDf.shape[1]) > int(Settings.cell_limit):
        log.info("This file has " + str(dataDf.shape[0] * dataDf.shape[1]) + " cells.")
        log.info(
            "The maximum number of cell that can be passed to this pipeline is "
            + str(Settings.cell_limit)
        )
        log.info(
            "File too big to handle!! Please remove the columns with low coverage and try again."
        )
        log.info(
            "Refer to this link: https://ehr-qc-tutorials.readthedocs.io/en/latest/process.html#large-file-handling"
        )
        return
        ...
        with tag("body"):
            if columns:
                with tag("div"):
                    with tag("h1"):
                        doc.asis(
                            '<svg xmlns="http://www.w3.org/2000/svg" width="25" height="25" fill="currentColor" class="bi bi-check-circle" viewBox="0 0 16 16"><path d="M8 15A7 7 0 1 1 8 1a7 7 0 0 1 0 14zm0 1A8 8 0 1 0 8 0a8 8 0 0 0 0 16z"/><path d="M10.97 4.97a.235.235 0 0 0-.02.022L7.477 9.417 5.384 7.323a.75.75 0 0 0-1.06 1.06L6.97 11.03a.75.75 0 0 0 1.079-.02l3.992-4.99a.75.75 0 0 0-1.071-1.05z"/></svg>'
                        )
                        with tag("span", klass="fs-4", style="margin: 10px;"):
                            text("Outliers plot")
                            log.info(
                                "Generating outlier plot for columns: " + str(columns)
                            )
...
def run(source_file, save_file, action, columns=[]):
    if columns:
        if action == "clean":
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="Outlier graphs using IRT ensemble technique"
    )
    ...
    parser.add_argument(
        "save_file", nargs=1, default="./", help="Path of the file to store the output"
    )
